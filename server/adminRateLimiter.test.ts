...
describe("adminRateLimiter — recordFailedAttempt", () => {
  it("allows attempts below the threshold", () => {
    for (let i = 1; i < MAX_ATTEMPTS; i++) {
      const result = recordFailedAttempt("10.0.0.1");
      expect(result.allowed).toBe(true);
      expect(result.attemptsLeft).toBe(MAX_ATTEMPTS - i);
    }
  });

  it("locks out on the MAX_ATTEMPTS-th failure", () => {
    for (let i = 0; i < MAX_ATTEMPTS - 1; i++) recordFailedAttempt("10.0.0.2");
    const result = recordFailedAttempt("10.0.0.2");
    expect(result.allowed).toBe(false);
    expect(result.attemptsLeft).toBe(0);
    expect(result.lockedUntil).toBeGreaterThan(Date.now());
  });

  it("keeps rejecting while locked out", () => {
    for (let i = 0; i < MAX_ATTEMPTS; i++) recordFailedAttempt("10.0.0.3");
    const result = recordFailedAttempt("10.0.0.3");
    expect(result.allowed).toBe(false);
  });
  ...
  it("resets the counter after the window expires", () => {
    ...
    for (let i = 0; i < MAX_ATTEMPTS - 1; i++) {
      recordFailedAttempt("10.0.0.4", now);
    }
    // Simulate time advancing past the window
    const afterWindow = now + WINDOW_MS + 1;
    const result = recordFailedAttempt("10.0.0.4", afterWindow);
    expect(result.allowed).toBe(true);
    expect(result.attemptsLeft).toBe(MAX_ATTEMPTS - 1);
  });

  it("tracks different IPs independently", () => {
    for (let i = 0; i < MAX_ATTEMPTS; i++) recordFailedAttempt("192.168.1.1");
    const otherResult = recordFailedAttempt("192.168.1.2");
    expect(otherResult.allowed).toBe(true);
  });
});

describe("adminRateLimiter — clearAttempts", () => {
  it("clears the counter so the IP can log in again", () => {
    for (let i = 0; i < MAX_ATTEMPTS - 1; i++) recordFailedAttempt("10.0.0.5");
    clearAttempts("10.0.0.5");
    const result = recordFailedAttempt("10.0.0.5");
    expect(result.allowed).toBe(true);
    expect(result.attemptsLeft).toBe(MAX_ATTEMPTS - 1);
  });
});
...
describe("adminRateLimiter — isLockedOut", () => {
  it("returns locked:false for a fresh IP", () => {
    expect(isLockedOut("10.0.0.6")).toEqual({ locked: false, lockedUntil: null });
  });

  it("returns locked:true after MAX_ATTEMPTS failures", () => {
    for (let i = 0; i < MAX_ATTEMPTS; i++) recordFailedAttempt("10.0.0.7");
    const { locked, lockedUntil } = isLockedOut("10.0.0.7");
    expect(locked).toBe(true);
    expect(lockedUntil).toBeGreaterThan(Date.now());
  });

  it("returns locked:false after the lockout expires", () => {
    const now = Date.now();
    for (let i = 0; i < MAX_ATTEMPTS; i++) recordFailedAttempt("10.0.0.8", now);
    const afterLockout = now + LOCKOUT_MS + 1;
    expect(isLockedOut("10.0.0.8", afterLockout)).toEqual({ locked: false, lockedUntil: null });
  });
});
...
describe("admin.login — rate limiting integration", () => {
  it("returns success and clears the counter on correct password", async () => {
    ...
    expect(isLockedOut("1.2.3.4").locked).toBe(false);
  });
